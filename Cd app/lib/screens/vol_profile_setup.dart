...
class VolProfileSetup extends StatefulWidget {
  static final String tag  ="/VolProfileSetup";
  @override
  _VolProfileSetupState createState() => _VolProfileSetupState();
}

class _VolProfileSetupState extends State<VolProfileSetup> {
  ...
  var findUsController = TextEditingController();
  ...
  Map<String,dynamic> caregoryId = {
    "Select Category" : "Select Category 1",
    "Select Category 2" : "Select Category 1",
  };
...
//          if(_profieCase == "1")
          Align(
            ...
            child: Container(
              ...
              margin: EdgeInsets.only( left: SizeConfig.blockSizeHorizontal * 10),
              child: Text('Choose Disability', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold),),
            ),
          ),
...
//              );
  ...
  void validation() async{
    ...
    if(dateText == "Choose Your BirthDate" && _profieCase == "1"){
      await showDialog(
        context: context,
    ...
    if(_selecteddisabilityList.length == 0 && _profieCase == "1") {
      await showDialog(
        context: context,
        builder: (ctx) => AlertDialog(
          title: Text(msg[ERR_TITLE]),
...
//          Map<String,dynamic> regData = Provider.of<AuthProvider>(context, listen: false).getInsertedVolInfoForReg;

          await Provider.of<AuthProvider>(context,listen: false).setRegVolInfo(
              password: passController.text.toString().trim(),
              phone: phoneController.text.toString().trim(),
              address: addressController.text.toString().trim(),
              email: emailController.text.toString().trim(),
              name: nameController.text.toString().trim(),
              disability_id: _selecteddisabilityList[0],
              categories_id: caregoryId[_categoryName].toString().trim(),
              institution_name: InstituationNameController.text.toString().trim(),
              age:dateText.toString(),
              gender: gender[isBtnSelected],
              occupation_name: occupationController.text.toString(),
              nid: "",
              find_us: findUsController.text.toString()
          );
          ...
          await Provider.of<AuthProvider>(context,listen: false).updateVolProfile(
              name: nameController.text.toString().trim(),
              address: addressController.text.toString().trim(),
              institution: InstituationNameController.text.toString().trim(),
              occupation: occupationController.text..toString().trim(),
              gender: gender[isBtnSelected],
              find_us: findUsController.text.toString().trim(),
